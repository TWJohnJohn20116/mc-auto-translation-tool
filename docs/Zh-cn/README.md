...
| 26.1?26.1.1?26.1.2?26.2????? JAR? | Forge | 25 |
...
主分支的 Fabric 1.16.5、Fabric 1.19.2 与 Forge 1.19.2 仍为开发中的独立源码目标；完成实机与服务器内
人工回归前不提供正式版下载。
...
全部构建线均通过干净构建，两个整合包通过真实 Fabric Loader 自动选版测试，
共享核心自测通过。详细验证层级见兼容性矩阵。

Fabric 版本按 `U` 打开设置。Forge 1.19.2/1.20.1/1.21.11 修改
