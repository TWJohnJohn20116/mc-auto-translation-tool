...
Forge 旧版运行时映射，以及 checksum 内容与覆盖范围。

发布流程会将 27 个精确构建产物缩减为 12 个可直接安装的 JAR：19 个 Fabric 实现会
放进一个由 Loader 自动选版的 JAR；四组 Forge 版本在 payload 兼容时扩宽范围；class
payload 相同的 Forge／NeoForge 1.20.1 则共用双加载器 JAR。不同 API 仍保持独立：
