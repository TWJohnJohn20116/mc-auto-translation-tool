...
**Does this affect chat signatures or require server-side installation?**

No. The mod replaces only the display copy during client rendering. It does not modify outgoing
messages, chat signatures, or server packets.
...
compatibility baseline. Back up the configuration before updating. When reporting untranslated
